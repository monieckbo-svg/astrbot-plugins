...
class QzoneMonitor:
    ...
    def _load_state(self) -> dict:
        default = {"seen_tids": [], "threads": {}, "post_contents": {}, "last_post_time": 0, "my_threads": {}, "pending_replies": []}
    ...
    async def _llm(self, prompt: str, persona_key: str = "comment_persona",
        ...
        try:
            ...
            if not provider or not isinstance(provider, Provider):
                logger.warning("[KaiQzone] LLM provider不可用")
                return None
    ...
    async def _get_recent_chat_summary(self, limit: int = 15) -> str:
        ...
        try:
            ...
            logger.error(f"[KaiQzone] 获取聊天记录失败: {e}")
            return ""
    ...
    async def start(self):
        ...
        logger.info(f"[KaiQzone] 监控启动 | 只看: {self.user_qq} | 间隔: {interval}s")
        try:
    ...
    async def _init_my_feeds_seen(self):
        ...
        try:
            ...
            logger.error(f"[KaiQzone] 初始化自己说说评论失败: {e}")
            return
        if not posts:
            return
    ...
    async def _retry_pending(self):
        ...
        for item in pending:
            ...
            if reply is None:
                ...
                if item["retries"] <= 10:  # 最多重试10次就放弃
                    ...
                    logger.warning(f"[KaiQzone] 放弃重试 type={item.get('type')} tid={item.get('tid')}")
                continue
            ...
            if item_type == "comment_new_post":
                ...
                if ok:
                    ...
                    self._state["threads"][tid] = [{"role": "kai", "content": reply}]
                    logger.info(f"[KaiQzone] 重试评论成功: {reply}")
                ...
                if ok:
                    ...
                    logger.info(f"[KaiQzone] 重试回复成功: {reply}")
                ...
                if cid and target_uin:
                    ok = await self.api.reply_comment(
                        item.get("owner_qq", self.kai_qq), tid, cid, target_uin, reply
                    )
                if not ok:
                    ok = await self.api.post_comment(item.get("owner_qq", self.kai_qq), tid, reply)
                if ok:
                    self.stats["replies"] += 1
                    logger.info(f"[KaiQzone] 重试回复 {item.get('speaker', '?')} 成功: {reply}")
        ...
        if retried > 0 or len(pending) != len(still_pending):
            self._save()
            logger.info(f"[KaiQzone] 重试完成: 成功{retried}条, 剩余{len(still_pending)}条待重试")
    ...
    async def _handle_new_post(self, post: dict):
        ...
        if comment is None:
            logger.warning(f"[KaiQzone] LLM生成评论失败，标记待重试 tid={tid}")
            ...
            self._save()
        elif await self.api.post_comment(self.user_qq, tid, comment):
    ...
    async def _check_replies(self, post: dict):
        ...
        def _index_cid(item):
            ...
            if u == self.user_qq and ctt and ctt not in cid_map:
                cid = str(item.get("commentid") or item.get("tid")
                          or item.get("comment_id") or item.get("id") or "")
                cid_map[ctt] = (cid, u)
                logger.info(f"[KaiQzone][调试] 收到她的评论 keys={list(item.keys())} 取到cid={cid!r}")
        ...
        for c in comments:
            ...
            if parent_uin == self.user_qq:
                ...
                _index_cid(c)
            # 子评论：只有当父评论是我(kai)发的，宝宝在我下面回复的，才算跟我说话
            for sub in (c.get("list_3") or c.get("replies") or []):
                sub_uin = str(sub.get("uin", ""))
                if sub_uin == self.user_qq and parent_uin == self.kai_qq:
        ...
        for reply_text in new:
            ...
            logger.info(f"[KaiQzone] 用户回复: {clean_text[:40]}")
            ...
            uname = self.config.get("user_name") or "用户"
            conv = "\n".join(
                f"{'Kai' if c['role'] == 'kai' else uname}: {c['content']}" for c in thread
            )
            ...
            if reply is None:
                logger.warning(f"[KaiQzone] LLM回复失败，标记待重试 tid={tid} 对方={uname}")
                self._state.setdefault("pending_replies", []).append({
                    "type": "reply_on_user_post",
                    "tid": tid, "owner_qq": self.user_qq,
                    "prompt": prompt, "persona_key": "comment_persona",
                    "cid": cid, "target_uin": target_uin,
                    "pending_thread_entry": {"role": "sweetie", "content": clean_text, "raw": reply_text},
                })
                self._save()
                continue

            # 优先用"回复评论"@到她，拿不到评论id就退回普通评论
            logger.info(f"[KaiQzone][调试] 准备回复 tid={tid} cid={cid!r} target_uin={target_uin!r}")
            ok = False
            if cid and target_uin:
                ok = await self.api.reply_comment(self.user_qq, tid, cid, target_uin, reply)
                logger.info(f"[KaiQzone][调试] reply_comment(@回复) 返回={ok}")
            if not ok:
                ok = await self.api.post_comment(self.user_qq, tid, reply)
                logger.info(f"[KaiQzone][调试] 退回 post_comment(普通评论) 返回={ok}")
    ...
    async def _check_my_feeds(self):
        """get_feeds(kai_qq) 检测谁在我自己说说下留言并回复。
        ...
        for post in posts:
            ...
            my_thread = self._state["my_threads"].get(tid, [])
            # 已处理过的留言键（兼容历史 sweetie 格式：没有 key 的就用内容兜底）
            known = set(e.get("key") for e in my_thread
                        if e.get("role") != "kai" and e.get("key"))
            ...
            for it in new_items:
                ...
                logger.info(f"[KaiQzone] {speaker} 在我说说下留言: {clean_text[:40]}")
                ...
                conv = "\n".join(
                    f"{'我' if e.get('role') == 'kai' else e.get('name', uname)}: {e['content']}"
                    for e in my_thread
                )
                ...
                if reply is None:
                    logger.warning(f"[KaiQzone] LLM回复失败，标记待重试 tid={tid} 对象={speaker}")
                    self._state.setdefault("pending_replies", []).append({
                        "type": "reply_on_my_post",
                        "tid": tid, "owner_qq": self.kai_qq,
                ...
                if it["cid"] and it["uin"]:
                    ok = await self.api.reply_comment(
                        self.kai_qq, tid, it["cid"], it["uin"], reply
                    )
                if not ok:
                ...
                if ok:
                    self.stats["replies"] += 1
                    logger.info(f"[KaiQzone] 回复 {speaker}: {reply}")
                else:
                    logger.warning(f"[KaiQzone] 回复失败 tid={tid} 对象={speaker}")
            ...
            if start_h <= now.hour < end_h:
                ...
                if hours_since_post >= min_gap:
                    ...
                    if (self._last_activity > 0
                        ...
                        triggered = True
                        logger.info("[KaiQzone] 对话冷却触发，考虑发说说")
                        await self._do_post()
                    ...
                    if not triggered:
                        ...
                        if hours_since_post >= random.uniform(min_h, max_h):
                            logger.info("[KaiQzone] 定时兜底触发，考虑发说说")
                            await self._do_post()
    ...
    async def _do_post(self):
        ...
        if content is None:
            logger.warning("[KaiQzone] 自动发说说LLM失败，跳过本轮")
            return

        if self._should_skip(content):
            logger.info(f"[KaiQzone] 这轮不发（回复: {content}）")
            return
    ...
    def get_status(self) -> str:
        ...
        return (f"[Kai的QQ空间]\n状态: {s}\n"
                f"Kai: {self.kai_qq} | User: {self.user_qq}\n"
